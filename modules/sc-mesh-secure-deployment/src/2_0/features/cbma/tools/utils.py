...
def setup_batman(batman_interface):
    if not is_interface_up(batman_interface): # Turn batman interface up if not up already
...
def get_mac_from_ipv6(ipv6_address, interface):
    try:
        ...
        subprocess.run(['ping', '-6', '-c', '1', f'{ipv6_address}%{interface}'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5)
